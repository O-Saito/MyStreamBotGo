...
func DoYouTubeRequest(req *http.Request) (*http.Response, error) {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		resp.Body.Close()
		helpers.Logf(helpers.Yellow, "[YT] Token expirado (401), tentando refresh...")

		// Faz refresh do token
		if err := RefreshToken(); err != nil {
			helpers.Logf(helpers.Red, "[YT] Falha ao fazer refresh: %s", err.Error())
			return nil, err
		}
		...
		if sqlErr != nil {
			helpers.Logf(helpers.Red, "[YT] Falha ao salvar token renovado: %s", sqlErr.Error())
		}
		...
		// Tenta novamente
		helpers.Logf(helpers.Green, "[YT] Retry de requisição com novo token")
		resp, err = http.DefaultClient.Do(req)
		if err != nil {
			return nil, err
		}
